...
//用bufferGeometry 创建酷炫的效果
for (let i=0;i<50;i++) {
    const geometry = new THREE.BufferGeometry();  // 设置一个空的几何体
    let arr = new Float32Array(9);
    for(let j=0;j<9;j++) {
        arr[j] = Math.random() * 10 - 5;
    }
    geometry.setAttribute("position", new THREE.BufferAttribute(arr, 3));  // 将顶点添加到空的几何体中 3个数值是一个顶点
    // 创建材质
    const color = new THREE.Color((Math.random() * 255), Math.random(), Math.random());
